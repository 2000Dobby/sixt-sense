...
const ArrowSVG = ({ rotation = 0 }: { rotation: number }) => (
  <svg width="200" height="200" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style={{ transform: `rotate(${rotation}deg)`, transition: 'transform 0.3s cubic-bezier(.4,2,.6,1)' }}>
    ...
    function handleOrientation(event: DeviceOrientationEvent) {
      ...
      if (compass !== null) {
        animationFrame = window.requestAnimationFrame(() => {
          const currentHeading = (compass + 360) % 360;
          setHeading(currentHeading); // Normalize
          setError(null);

          const targetRotation = (targetAngle - currentHeading + 360) % 360;
